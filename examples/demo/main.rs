...
const BLACK: RGBA = RGBA::rgba(0, 0, 0, 255);
const WHITE: RGBA = RGBA::rgba(255, 255, 255, 255);

fn my_to_rgba(name: &str) -> Option<RGBA> {
    match name {
        "white" => Some(WHITE),
        "red" => Some((255, 92, 92, 255).into()),
        "blue" => Some((192, 192, 255, 255).into()),
        _ => to_rgba(name),
    }
}
...
impl Screen for DoryenDemo {
    ...
    fn render(&mut self, app: &mut AppContext) {
        if self.loaded {
            ...
            draw::colored(self.con.buffer_mut()).align(TextAlign::Center).to_rgba(&my_to_rgba).print(
